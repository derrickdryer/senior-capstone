...
exports.getNotificationById = async (ctx) => {
    try {
        const [rows] = await pool.query("SELECT * FROM notifications WHERE notification_id = ?", [ctx.params.id]);
        if (rows.length === 0) {
            ctx.status = 404;
...
exports.createNotification = async (ctx) => {
    try {
        const { user_id, notification_type, content, sent_date, read_status } = ctx.request.body;
        ...
        ctx.status = 201;
        ctx.body = { message: "Notification created successfully", notification_id: result.insertId };
    } catch (error) {
...
exports.updateNotification = async (ctx) => {
    try {
        ...
        const [result] = await pool.query(
            ...
            [user_id, notification_type, content, sent_date, read_status, ctx.params.id]
        );
...
exports.deleteNotification = async (ctx) => {
    try {
        const [result] = await pool.query("DELETE FROM notifications WHERE notification_id = ?", [ctx.params.id]);
        if (result.affectedRows === 0) {
            ctx.status = 404;
            ctx.body = { error: "Notification not found" };
            return;
