...
exports.getPaymentById = async (ctx) => {
    try {
        const [rows] = await pool.query("SELECT * FROM payments WHERE payment_id = ?", [ctx.params.id]);
        if (rows.length === 0) {
            ctx.status = 404;
...
exports.createPayment = async (ctx) => {
    try {
        const { lease_id, payment_date, amount, payment_method, status } = ctx.request.body;
        ...
        ctx.status = 201;
        ctx.body = { message: "Payment created successfully", payment_id: result.insertId };
    } catch (error) {
...
exports.deletePayment = async (ctx) => {
    try {
        const [result] = await pool.query("DELETE FROM payments WHERE payment_id = ?", [ctx.params.id]);
        if (result.affectedRows === 0) {
            ctx.status = 404;
            ctx.body = { error: "Payment not found" };
            return;
