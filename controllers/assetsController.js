...
exports.getAssetById = async (ctx) => {
    try {
        const [rows] = await pool.query("SELECT * FROM assets WHERE property_id = ?", [ctx.params.id]);
        if (rows.length === 0) {
            ctx.status = 404;
...
exports.createAsset = async (ctx) => {
    try {
        const { address, city, state, postal_code, num_apartments } = ctx.request.body;
        ...
        ctx.status = 201;
        ctx.body = { message: "Asset created successfully", property_id: result.insertId };
    } catch (error) {
...
exports.deleteAsset = async (ctx) => {
    try {
        const [result] = await pool.query("DELETE FROM assets WHERE property_id = ?", [ctx.params.id]);
        if (result.affectedRows === 0) {
            ctx.status = 404;
            ctx.body = { error: "Asset not found" };
            return;
