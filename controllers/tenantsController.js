...
exports.getTenantById = async (ctx) => {
    try {
        const [rows] = await pool.query("SELECT * FROM tenants WHERE tenant_id = ?", [ctx.params.id]);
        if (rows.length === 0) {
            ctx.status = 404;
...
exports.createTenant = async (ctx) => {
    try {
        ...
        ctx.status = 201;
        ctx.body = { message: "Tenant created successfully", tenant_id: result.insertId };
    } catch (error) {
...
exports.deleteTenant = async (ctx) => {
    try {
        const [result] = await pool.query("DELETE FROM tenants WHERE tenant_id = ?", [ctx.params.id]);
        if (result.affectedRows === 0) {
            ctx.status = 404;
            ctx.body = { error: "Tenant not found" };
            return;
