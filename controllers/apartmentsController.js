...
exports.getApartmentById = async (ctx) => {
    try {
        const [rows] = await pool.query("SELECT * FROM apartments WHERE apartment_id = ?", [ctx.params.id]);
        if (rows.length === 0) {
            ctx.status = 404;
...
exports.createApartment = async (ctx) => {
    try {
        const { property_id, unit_number, floor, bedrooms, bathrooms, square_footage, rent_amount } = ctx.request.body;
        ...
        ctx.status = 201;
        ctx.body = { message: "Apartment created successfully", apartment_id: result.insertId };
    } catch (error) {
...
exports.updateApartment = async (ctx) => {
    try {
        ...
        const [result] = await pool.query(
            ...
            [property_id, unit_number, floor, bedrooms, bathrooms, square_footage, rent_amount, ctx.params.id]
        );
...
exports.deleteApartment = async (ctx) => {
    try {
        const [result] = await pool.query("DELETE FROM apartments WHERE apartment_id = ?", [ctx.params.id]);
        if (result.affectedRows === 0) {
            ctx.status = 404;
            ctx.body = { error: "Apartment not found" };
            return;
