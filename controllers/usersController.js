...
exports.getUserById = async (ctx) => {
    try {
        const [rows] = await pool.query("SELECT * FROM users WHERE user_id = ?", [ctx.params.id]);
        if (rows.length === 0) {
            ctx.status = 404;
...
exports.createUser = async (ctx) => {
    try {
        const { property_id, role, username, password, phone_number, mfa_secret } = ctx.request.body;
        ...
        ctx.status = 201;
        ctx.body = { message: "User created successfully", user_id: result[0].insertId };
    } catch (error) {
...
exports.updateUser = async (ctx) => {
    try {
        ...
        await pool.query(
            ...
            [property_id, role, username, password, phone_number, mfa_secret, ctx.params.id]
        );
        ctx.status = 200;
